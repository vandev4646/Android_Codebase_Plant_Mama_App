...
//AUTHOR: GOT MOST OF THE CODE FROM: https://github.com/Everyday-Programmer/Android-Camera-using-CameraX/blob/main/app/src/main/res/layout/activity_main.xml
        ...
        imageCapture.takePicture(outputFileOptions, Executors.newCachedThreadPool(), new ImageCapture.OnImageSavedCallback() {
            ...
            public void onImageSaved(@NonNull ImageCapture.OutputFileResults outputFileResults) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(Camera.this, "Image saved at: " + file.getPath(), Toast.LENGTH_SHORT).show();
                    }
                });
                startCamera(cameraFacing);
            }
            ...
            public void onError(@NonNull ImageCaptureException exception) {
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        Toast.makeText(Camera.this, "Failed to save: " + exception.getMessage(), Toast.LENGTH_SHORT).show();
                    }
                });
                startCamera(cameraFacing);
            }
    ...
    private void setFlashIcon(androidx.camera.core.Camera camera) {
        if (camera.getCameraInfo().hasFlashUnit()) {
            ...
            runOnUiThread(new Runnable() {
                @Override
                public void run() {
                    Toast.makeText(Camera.this, "Flash is not available currently", Toast.LENGTH_SHORT).show();
                }
            });
        }
    }
