...
import com.android.example.plantmamaapp_v3.FIVE_SECONDS
import com.android.example.plantmamaapp_v3.ONE_DAY
import com.android.example.plantmamaapp_v3.R
import com.android.example.plantmamaapp_v3.SEVEN_DAYS
import com.android.example.plantmamaapp_v3.THIRTY_DAYS
...
fun AddReminder(
    ...
    var textReminderName by remember { mutableStateOf("Reminder Name") }
    ...
    Card(
        ...
        Column(
            ...
            Text(text = stringResource(R.string.add_reminder), modifier = Modifier.padding(16.dp), style = MaterialTheme.typography.labelLarge)
        }
        ...
        Column(
            ...
            OutlinedTextField(
                ...
                onValueChange = {
                    reminderViewModel.updateUiState(reminderUiState.reminderDetails.copy(title = it))
            ...
            Row(
                ...
                TextButton(
                    onClick = {
                        val reminderIdentifier = viewModel.currentPlant.name + currentTimeMillis().toString()
                        val reminderWM = ReminderWM(duration = totalDelay.toLong(), unit = TimeUnit.SECONDS, plantName = viewModel.currentPlant.name, reminderIdentifier = reminderIdentifier)
                        reminderViewModel.updateUiState(reminderUiState.reminderDetails.copy(wmIdentifier = reminderIdentifier))
                        reminderViewModel.updateUiState(reminderUiState.reminderDetails.copy(plantID = viewModel.currentPlant.id.toString()))
...
fun DateTimePickerComponent(reminderViewModel: ReminderEntryViewModel = viewModel(factory = AppViewModelProvider.Factory)) {
    ...
    val date = remember {
        Calendar.getInstance().apply {
           ...
           // set(Calendar.MONTH, LocalDate.now().month.value)
    ...
    val minFormatter = remember { SimpleDateFormat("mm", Locale.getDefault()) }

    var currentTime = currentTimeMillis()
    var customTime  = remember {datePickerState.selectedDateMillis }
    val delay = remember { customTime?.minus(currentTime) ?: 0 }
        ...
        Row {
            ...
            Button(
                ...
                if (selectedTime != null){
                    delayHourS = ((selectedTime!!.hour - currentHour) * 3600.4566211200049111).toInt()
                    ...
                    cal.isLenient = false
                    reminderViewModel.updateUiState(reminderUiState.reminderDetails.copy(time = formatter.format(cal.time)))
    ...
    if (showDatePicker) {
        DatePickerDialog(
            ...
            confirmButton = {
                TextButton(
                    onClick = {
                        delayYearS = ((yearFormatter.format(datePickerState.selectedDateMillis).toInt()-currentYear)* 3.154e+7).toInt()
                        delayMonthS = ((monthFormatter.format(datePickerState.selectedDateMillis).toInt()-currentMonth)*2628336.2137829).toInt()
                        delayDayS = ((dayFormatter.format(datePickerState.selectedDateMillis).toInt()-currentDay)*86410.958906880114228).toInt()
                        ...
                        pastDate = true
                        dateSelected = true
                        reminderViewModel.updateUiState(reminderUiState.reminderDetails.copy(date = dateFormatter.format(datePickerState.selectedDateMillis)))
...
@Preview()
@Composable
fun displayAddReminder(){
    PLantMamaTheme {
        AddReminder(onDismissRequest = { /*TODO*/ }) {
